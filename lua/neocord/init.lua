...
function neocord:get_project_name(file_path)
  ...
  if is_git_repo then
    -- Git repository found, normalize path for Windows if needed
    if self.os.name == "windows" then
  ...
  if project_path:find("error.*") then -- 'jj root' outputs 'Error: Not a repo' or similar
  ...
  if is_jj_repo then
    -- JJ repository found, normalize path for Windows if needed
    if self.os.name == "windows" then
      project_path = project_path:gsub("/", [[\]])
    end
    return self.get_filename(project_path, self.os.path_separator), project_path
  ...
  -- If neither Git nor JJ repo was found
